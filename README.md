...
Yolomi.zip
Zip Archive
We can chnge the bbox.... as per the clients wish the colour ... 
ClassPanel(1).jsx
JavaScript
src.zip
Zip Archive
README.md
File
update the Readme... add the color circle

Yes. I’ll update the README’s Features section to clearly mention the new clickable color circle/swatch, where clients can choose a custom bounding-box color.

Your current README already has the per-class color feature documented at lines 96–99.

I can’t save over the uploaded file in-place from here, but you can replace this line:
...
- **Custom per-class colors** — each class has a clickable color circle/swatch, allowing users to choose any bounding-box color they prefer. Colors remain stable across sessions, and each box uses its class color for quick visual identification.

Then in PowerShell:

cd D:\Yolomi\yolomi

Replace your local README.md with the updated one, then:

git add README.md
git commit -m "Update README with custom class colors"
git push origin main

After that, the README on GitHub will show the new custom color circle feature.
...
Yes — for your README, replace the current Per-class colors bullet with this updated version:

Updated README feature
Per-class colors — each class has a clickable color circle/swatch that lets the user choose a custom color for that class. The selected color is applied to the class's bounding boxes, making different object classes easy to distinguish visually.

So in your README, this:

- **Per-class colors**, auto-assigned and stable across sessions, with a colored label tag on every box so classes stay visually distinguishable at a glance.

becomes the updated version above.

Your README already has the Features section where this belongs.

After saving README.md in D:\Yolomi\yolomi, run:

git add README.md
git commit -m "Update README with custom class colors"
git push origin main

This will update the README on GitHub.

update the readme file

Done. I updated the README to include the clickable color circle/swatch feature for custom bounding-box colors.

Download the updated README.md

Replace your existing D:\Yolomi\yolomi\README.md with this file, then run:
...
Requires Node.js (includes npm). Check with node -v if you're not sure it's installed.
...
Install Node.js (skip if already installed): download from nodejs.org, choose the LTS version, and run the installer. Confirm it worked by opening a terminal and running:
...
If the browser doesn't open automatically, copy the URL printed in the terminal (something like http://localhost:4173) and paste it into your browser manually.
Use Chrome or Edge for the best experience — these support saving labels directly to a folder on disk. Other browsers still work, but fall back to a zip download instead of direct folder access.
...
Licensed under a custom license — free to use, but modification and redistribution of modified versions are not permitted. See the LICENSE file for full terms.